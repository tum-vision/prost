...
template<typename T>
__global__ void BlockIdKronSparseKernel(
    T *result,
    const T *rhs,
    size_t diaglength,
    size_t nrows,
...
template<typename T>
BlockIdKronSparse<T>::BlockIdKronSparse(size_t row, size_t col, size_t nrows, size_t ncols)
    : Block<T>(row, col, nrows, ncols)
{
}

template<typename T>
BlockIdKronSparse<T> *BlockIdKronSparse<T>::CreateFromCSC(
    size_t row,
    size_t col,
    size_t diaglength,
    int m,
...
{
  BlockIdKronSparse<T> *block = new BlockIdKronSparse<T>(row, col, ((size_t)m) * diaglength, ((size_t)n) * diaglength);
...
template<typename T>
void BlockIdKronSparse<T>::Initialize()
{
  // forward
...
template<typename T>
T BlockIdKronSparse<T>::row_sum(size_t row, T alpha) const
{
  row = row % mat_nrows_;
...
template<typename T>
T BlockIdKronSparse<T>::col_sum(size_t col, T alpha) const
{
  col = col % mat_ncols_;
...
template<typename T>
size_t BlockIdKronSparse<T>::gpu_mem_amount() const
{
...
template<typename T>
void BlockIdKronSparse<T>::EvalLocalAdd(
    ...
    const typename device_vector<T>::const_iterator& rhs_end)
{
  ...
  if(error != cudaSuccess)
  {
    ...
    std::stringstream ss;
    ss << "BlockIdKronSparse: CUDA error: " << cudaGetErrorString(error) << std::endl;
    throw Exception(ss.str());
  }
}

template<typename T>
void BlockIdKronSparse<T>::EvalAdjointLocalAdd(
    const typename device_vector<T>::iterator& res_begin,
    const typename device_vector<T>::iterator& res_end,
    const typename device_vector<T>::const_iterator& rhs_begin,
    const typename device_vector<T>::const_iterator& rhs_end)
...
template class BlockIdKronSparse<float>;
template class BlockIdKronSparse<double>;
