...
// Optimizations:
// - read rhs in shared memory
// - store ind, ptr and val in constant memory
template<typename T>
__global__ void BlockSparseKronIdForward(
    T *result,
    const T *rhs,
    size_t diaglength,
    size_t nrows,
...
void BlockSparseKronId<T>::Initialize()
{
  ...
  //std::cout << gpu_mem_amount() << " bytes." << std::endl;
}
