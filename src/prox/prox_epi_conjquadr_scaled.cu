...
template<typename T>
__global__
void ProxEpiConjQuadrKernelScaled(T *d_arg,
  T *d_res,
...
template<typename T>
ProxEpiConjQuadrScaled<T>::ProxEpiConjQuadrScaled(size_t index,
  size_t count,
  bool interleaved,
...
template<typename T>
ProxEpiConjQuadrScaled<T>::~ProxEpiConjQuadrScaled() {
  Release();
}

template<typename T>
bool ProxEpiConjQuadrScaled<T>::Init() {
...
template<typename T>
void ProxEpiConjQuadrScaled<T>::Release() {
  for(size_t i = 0; i < PROX_EPI_CONJQUADR_NUM_COEFFS; i++) {
...
template<typename T>
void ProxEpiConjQuadrScaled<T>::EvalLocal(T *d_arg,
                                    T *d_res,
                                    T *d_tau,
                                    T tau,
                                    bool invert_tau)
{
  ...
  ProxEpiConjQuadrKernelScaled<T>
      <<<grid, block>>>(
        d_arg,
        d_res,
        coeffs_dev_,
...
template class ProxEpiConjQuadrScaled<float>;
template class ProxEpiConjQuadrScaled<double>;
