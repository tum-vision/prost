function [passed] = test_linop_id_kron_dense()

    diaglength = 271;
    nrows = 49;
    ncols = 83;
    K_mat = sparse(randn(nrows, ncols));
    
    block_fun = prost.block.id_kron_dense(K_mat, diaglength);
    make_block_id_kron_dense_1 = ...
        block_fun(0, ...
                  0, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_id_kron_dense_2 = ...
        block_fun(size(K_mat, 1) * diaglength, ...
                  0, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_id_kron_dense_3 = ...
        block_fun(size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_id_kron_dense_4 = ...
        block_fun(0, ...
                  size(K_mat, 2) * diaglength, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);
    ...
    linop = { ...
        make_block_id_kron_dense_1{1}; ...
        make_block_id_kron_dense_2{1}; ...
        make_block_id_kron_dense_3{1}; ...
        make_block_id_kron_dense_4{1}; ...
            };
    
    K = kron(speye(diaglength), K_mat);
    K = [K K; K K];
