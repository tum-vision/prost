function [passed] = test_linop_dense_kron_id()

    diaglength = 122;
    nrows = 13;
    ncols = 14;
    K_mat = randn(nrows, ncols);
    % sum(sum(abs(K_mat)>0))
    
    block_fun = prost.block.dense_kron_id(K_mat, diaglength);
    make_block_dense_kron_id_1 = ...
        block_fun(0, ...
                  0, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_dense_kron_id_2 = ...
        block_fun(size(K_mat, 1) * diaglength, ...
                  0, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_dense_kron_id_3 = ...
        block_fun(size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);

    make_block_dense_kron_id_4 = ...
        block_fun(0, ...
                  size(K_mat, 2) * diaglength, ...
                  size(K_mat, 1) * diaglength, ...
                  size(K_mat, 2) * diaglength);
    ...
    linop = { ...
        make_block_dense_kron_id_1{1}; ...
        make_block_dense_kron_id_2{1}; ...
        make_block_dense_kron_id_3{1}; ...
        make_block_dense_kron_id_4{1}; ...
            };
    
    K = full(kron(K_mat, speye(diaglength)));
    K = [K K; K K];
